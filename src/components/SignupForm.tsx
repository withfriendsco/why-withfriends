...
const SignupForm = () => {
	...
	const submitForm = async () => {
		...
		const response = await fetch(url, {
			...
			body: `${encodeURIComponent("EMAIL")}=${encodeURIComponent(emailAddress)}&${encodeURIComponent("FNAME")}=${encodeURIComponent(personName)}`,
		})
		console.log(response)
		if (response.status == 200) {
			setSubmitMessage(<SubmitData message="Success! Thanks for joining." />)
		} else {
			setSubmitMessage(<SubmitData message='Problem signing you up. Press "Get Started" on the top right to try another way.' />)
		}
		setLoading(false)
	}
	...
	return (
		<div>
			...
			<div className="w-full flex justify-center mt-8">
				<Button variant="salmon" loading={loading} onClick={submitForm}>Sign Up</Button>
			</div>
		</div>
	)
}

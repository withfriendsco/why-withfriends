...
const getTokenFromCode = async code => {
  ...
  const response = await fetch(facebookCodeURL, {
    method: 'GET'
  });
...
const getAppAccessToken = async () => {
  ...
  let response = await fetch(`${facebookAccessTokenURL}?${params}`, {
    method: 'GET'
  });
...
const getUserInfo = async (token, userId) => {
  ...
  const response = await fetch(`https://graph.facebook.com/v9.0/${userId}?${params}`, {
    method: 'GET'
  });
...
const userCreate = async userInfo => {
  const response = await fetch(process.env.GATSBY_GRAPHQL_API || "http://localhost:4002/graphql", {
    method: 'POST',
    headers: {
...
exports.handler = async function (event, context) {
  ...
  if (token[0] !== process.env.GATSBY_FB_APP_ID) return {
    statusCode: 400
  };
  const verifyTokenResponse = await verifyToken(tokenResponse.access_token);
  if (!verifyTokenResponse) return {
    statusCode: 400
  };
